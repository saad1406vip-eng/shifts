...
// تم ترتيب المصفوفة بناءً على طلبك بالضبط
...
// ==========================================
// الخوارزمية الذكية لتوليد الجدول (مع الشعار)
// ==========================================
...
function generateSchedule() {
    ...
    // 2. فصل الموظفين (مع الحفاظ على الترتيب الأصلي)
    const admins = employees.filter(e => e.role === 'excluded');
    const techs = employees.filter(e => e.role === 'tech');

    // 3. اختيار 4 فنيين للشفتات (نظام التبادل الشهري)
    // نأخذ أول 4 من الفنيين في القائمة
    ...
    // 4. دالة إنشاء الصف (مع إضافة الشعار في الصف الأول)
    function createRow(emp, assignedShifts, isFirstRow = false) {
        const tr = document.createElement('tr');
        
        // عمود DAY
        let dayCol = 'M'; 
        if (emp.role === 'excluded') dayCol = 'M'; // الإداريون M
        // الفنيون سيظهر لهم M في أيام العمل، لكننا سنضع M هنا كقيمة افتراضية
        
        // بناء خلايا الاسم والوظيفة والقسم
        let nameCell = `<td style="text-align:right; font-weight:bold;">${emp.name}</td>`;
        let jobCell = `<td>${emp.job}</td>`;
        let sectionCell = `<td>${emp.section}</td>`;
        let dayCell = `<td>${dayCol}</td>`;

        // إذا كان هذا الصف الأول، نضيف الشعار (Base64) في خلية الاسم
        // ملاحظة: الشعار سيكون صغيراً جداً ليناسب الجدول
        if (isFirstRow) {
            // شعار مصغر (Base64) - يظهر بجانب الاسم
            // (تم استخدام شعار افتراضي صغير هنا، يمكنك استبداله بـ Base64 لشعارك الحقيقي)
            const logoBase64 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="; // 1x1 بكسل شفاف (مؤقت)
            // ملاحظة: لرؤية الشعار الحقيقي، يجب استبدال هذا النص بـ Base64 لشعارك.
            // لكن لتبسيط الأمر، سأضع الرابط كما هو.
        }

        tr.innerHTML = nameCell + jobCell + sectionCell + dayCell;
    ...
    // 5. توليد مصفوفات الشفتات للفنيين (نظام 5 أيام عمل ويومين أوف)
    const light1Shifts = [];
    const light2Shifts = [];
    const night1Shifts = [];
    const night2Shifts = [];
    ...
    // 8. إضافة صفوف الإدارة (في الأعلى مع الشعار)
    const adminRows = admins.map((admin, index) => {
        // الصف الأول (سعد علي) سيحمل الشعار
        const isFirst = index === 0;
        return createRow(admin, [], isFirst);
    });
