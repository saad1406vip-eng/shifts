...
// ==========================================
// توليد الجدول (مع احترام الترتيب اليدوي)
// ==========================================
function generateSchedule() {
    ...
    // 3. ترتيب الموظفين حسب employeeOrder (لا نفرز admin عن tech)
    const sortedEmployees = [...employees].sort((a, b) => {
        const orderA = employeeOrder[a.id] || 999;
        const orderB = employeeOrder[b.id] || 999;
        return orderA - orderB;
    ...
    // 6. مصفوفات الشفتات
    const light1Shifts = [], light2Shifts = [], night1Shifts = [], night2Shifts = [];
    ...
    // 7. شفتات الفنيين العاديين (تُخزن مؤقتاً)
    const regularTechShifts = {};
    ...
    techs.forEach(tech => {
        ...
        regularTechShifts[tech.id] = shifts;
    });

    // 8. بناء الجدول بالترتيب اليدوي
    sortedEmployees.forEach(emp => {
        if (emp.role === 'excluded') {
            scheduleBody.appendChild(createRow(emp, []));
        } else if (emp.id === lightTech1.id) {
            scheduleBody.appendChild(createRow(emp, light1Shifts));
        } else if (emp.id === lightTech2.id) {
            scheduleBody.appendChild(createRow(emp, light2Shifts));
        } else if (emp.id === nightTech1.id) {
            scheduleBody.appendChild(createRow(emp, night1Shifts));
        } else if (emp.id === nightTech2.id) {
            scheduleBody.appendChild(createRow(emp, night2Shifts));
        } else {
            scheduleBody.appendChild(createRow(emp, regularTechShifts[emp.id] || []));
        }
    });
}
